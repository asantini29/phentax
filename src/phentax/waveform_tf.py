...
class IMRPhenomTHM_TF:
    ...
    # @jax.jit(static_argnums=[0,16])
    # def get_tf_fresnel_waveform_subsample_TF(self,
    #                             time_grid: Array,
    #                             frequency_grid: Array,
    #                             m1: float | Array,
    #                             m2: float | Array,
    ...
    #     subsampling_grid = time_grid + dT/2

    #     # Work out what OUR time grid corresponds to in mass units for each binary 
    #     time_grids_mass_units = jax.vmap(second_to_mass, in_axes=(None, 0))(time_grid, wf_params.total_mass)
    #     subsampling_grids_mass_units = jax.vmap(second_to_mass, in_axes=(None, 0))(subsampling_grid, wf_params.total_mass)
    ...
    #     amplitudes_subsampling, phases_subsampling = jax.vmap(self._compute_all_modes, in_axes = (0, None, 0, 0, 0))(
    #         subsampling_grids_mass_units,
    #         new_mask,
    #         wf_params,
    #         amplitude_coeffs_22,
    #         phase_coeffs_22,
    ...
    #     # amp_factor shape: [num_sources] -> need to reshape for broadcasting
    #     amplitudes *= wf_params.amp_factor[:, None, None]
    #     amplitudes_subsampling *= wf_params.amp_factor[:, None, None]
    ...
    #     # Back to sensible and positive time grid
    #     time_grid -= min_time_s_units
    #     time_grid_subsampling = subsampling_grid - min_time_s_units
    ...
    #             t_0 = time_grid[time_index]
    #             t_1 = time_grid[time_index+1]
    #             t_s = time_grid_subsampling[time_index] # This is the subsampling point between t_0 and t_1.
    ...
    #             Amps_subsampling = amplitudes_subsampling[:,:, time_index] # Source, #Mode , # Time
    #             Phases_subsampling = phases_subsampling[:,:, time_index]   # Source, #Mode , # Time
    ...
    #             f_0_subsampling = jax.vmap(
    #                 lambda t, eta, pc: jax.vmap(lambda p: imr_omega(t, eta, p))(pc)
    #             )(t_s + min_time_s_units, wf_params.eta, overall_phase_coeffs) / (2 * jnp.pi)  # IN MASS UNITS

    #             f_dot_subsampling = jax.vmap(
    #                 lambda t, eta, pc: jax.vmap(lambda p: imr_omega_dot(t, eta, p))(pc)
    #             )(t_s + min_time_s_units, wf_params.eta, overall_phase_coeffs) / (2 * jnp.pi)  # IN MASS UNITS

    #             f_0_subsampling = jax.vmap(mass_to_hz, in_axes=(0, 0))(f_0_subsampling, wf_params.total_mass)
    #             f_dot_subsampling = jax.vmap(df_dt_to_Hz_squared, in_axes=(0, 0))(f_dot_subsampling, wf_params.total_mass)
